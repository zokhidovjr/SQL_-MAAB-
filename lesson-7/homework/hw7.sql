...
SELECT COUNT(DISTINCT Category) AS UniqueC FROM Products;

SELECT SUM(SaleAmount) AS Total FROM Sales WHERE ProductID = 7;

SELECT AVG(Age) AS AvgAge FROM Employees;

SELECT DepartmentName,COUNT(*) AS Emps FROM Employees
...
SELECT ProductID,SUM(SaleAmount) AS Totalsale, AVG(SaleAmount) AS AvgSale
FROM Sales GROUP BY ProductID;

SELECT DepartmentName, COUNT(*) Emps FROM Employees
GROUP BY DepartmentName
HAVING DepartmentName = 'HR';

SELECT DepartmentName, MIN(Salary) Lowest, MAX(salary) Highest
FROM Employees
GROUP BY DepartmentName;
...
SELECT DepartmentName, AVG(Salary) AvgSalary, COUNT(*) Emps
FROM Employees
GROUP BY DepartmentName;
...
SELECT YEAR(SaleDate) AS SalesYear, SUM(SaleAmount) As TotalSale FROM Sales
...
SELECT 
    CustID,
    SUM(CASE WHEN Freight > 50 THEN Freight ELSE 0 END) AS TotalAmountAbove50,
...
COUNT(DISTINCT ProductID) AS UniquePrdct
FROM Orders
...
SELECT YEAR(OrderDate) AS SalesYear, 
MIN(Quantity) AS MinQ,MAX(Quantity) AS MaxQ
FROM Orders
